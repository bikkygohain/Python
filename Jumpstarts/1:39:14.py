...
# Built in function - are provided by python language itself
print("-------------------------------------------------------")
print("User Input")
print("-------------------------------------------------------")

user_input = input("Hey user, enter a number of days and I will convert it into hours !\n")


def user_input_test(num_of_days):
...
my_input_var = user_input_test(20)
print(my_input_var)
calculated_value = user_input_test(user_input_number)
...
def cond_days_to_units(num_of_days):
    if num_of_days > 0:
        return f"{num_of_days} days are {num_of_days * calculation_to_units} {name_of_unit}"
    elif num_of_days == 0:
        return "You have entered a 0, please enter a valid no !"
    else:
        return "You entered a negative value, so no conversion for you!"

cond_user_input = input("Hey user, enter a number of days and I will convert it to hours\n")
# Nested function calls
# Type checking for errors in case of wrong inputs
if cond_user_input.isdigit():
    cond_user_input_number = int(cond_user_input)
    cond_calculated_value = cond_days_to_units(cond_user_input_number)
    print(cond_calculated_value)
else:
    print("Your input is not a number. Don't ruin my program!")

# Note: Conditonal can be TRUE or FALSE
